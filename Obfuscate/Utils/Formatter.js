...
function CreateLuaParser(text) {
	...
	function prefixexpr(locals, upvals) {
		...
		if (tk.Source == "(") {
			...
			print(debugMark());
			let a = `${getTokenStartPosition(tk)}: Unexpected symbol. ${
				tk.Type
			} ${tk.Source}`;
			throw a;
		}
	...
	function exprstat(locals, upvals) {
		...
		if (ex.Type == "MethodExpr" || ex.Type == "CallExpr") {
			...
			while (peek().Source == ",") {
				...
				if (
					lhsPart.Type == "MethodExpr" ||
					lhsPart.Type == "CallExpr"
				) {
			...
			node = MkNode({
				...
				GetLastToken: () =>
					node.Rhs[node.Rhs.length - 1].GetLastToken(),
			});
			...
			if (funcStat.NameChain.length > 1) {
				throw (
					getTokenStartPosition(
						funcStat.Token_NameChainSeperator[0]
					) + ": `(` expected."
				);
			}
	...
	block = function (a, b) {
		...
		while (!isLast && !isBlockFollow()) {
			if (thing && thing == peek()) {
				print(
					`INFINITE LOOP POSSIBLE ON STATEMENT ${thing.Source} :`,
					thing
				);
			}
			thing = peek();
...
function AddVariableInfo(ast) {
	...
	visitor.VariableExpr = function (expr) {
		expr.Variable = referenceVariable(
			expr.Token.Source,
			function (newName) {
		...
		if (expr.Type == "BinopExpr") {
			...
			if (expr.Type == "HashLiteral") {
				expr.Token.Source =
					'"' +
					hashString(
						`${expr.Token.Source.substring(
							1,
							expr.Token.Source.length - 1
						)}`
					) +
					'"';
				expr.Type = "StringLiteral";
...
function StripAst(ast) {
	...
	function joint(tokenA, tokenB, shit = false) {
		...
		if (
			(lastCh == "-" && firstCh == "-") ||
			(AllIdentChars.includes(lastCh) &&
				AllIdentChars.includes(firstCh)) ||
			(shit && lastCh == ")" && firstCh == "(")
		) {
		...
		if (stat.Type == "StatList") {
			...
			for (i = 0; i <= stat.StatementList.length; i++) {
				...
				if (lastChStat != null) {
					...
					if (!stat.SemicolonList[i - 1]) {
						joint(
							lastChStat.GetLastToken(),
							chStat.GetFirstToken(),
							true
						);
					}
				}
			...
			joint(
				stat.FunctionStat.Token_Function,
				stat.FunctionStat.NameChain[0]
			);
			joint(
				stat.FunctionStat.NameChain[0],
...
function SolveMath(ast) {
	...
	function solvebinop(operator, left1, right1) {
		...
		let a =
			lhs.Token ||
			(lhs.Expression != null && lhs.Expression.Token) ||
			null;
		let b =
			rhs.Token ||
			(rhs.Expression != null && rhs.Expression.Token) ||
			null;
		...
		if (_var.Scope) {
			allVariables.forEach((otherVar) => {
				if (!otherVar.Renamed) {
					if (
						!otherVar.Scope ||
						otherVar.Scope.Depth < _var.Scope.Depth
					) {
...
luaminp.Minify = function (
	scr,
	options = { SolveMath: false, RenameVariables: false }
) {
...
luaminp.UltraBeautify = function (scr) {
	let minified = luaminp.Minify(scr);
	let beautified = luaminp.Beautify(minified);
	return beautified;
};
...
try {
	if (module != null && module.exports != null) {
		...
		module.exports.UltraBeautify = luaminp.UltraBeautify;
	}
} catch (err) {}
